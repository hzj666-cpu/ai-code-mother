...
const onFinish = async () => {
  ...
  try {
    ...
    if (res.data.code === 0) {
      ...
      // 注册成功不自动登录，回到登录页（后端 /user/register 只返回 id）
      setTimeout(() => router.replace('/user/login'), 800)
    } else {
      message.error('注册失败：' + (res.data.message ?? '请检查输入'))
    }
  } catch (e) {
    ...
    <div class="register-container">
      ...
      <div class="card-panel">
        <div class="register-card">
          ...
          <p class="register-card__subtitle">注册即可开始构建你的应用</p>
          ...
          >
            ...
            <a-form-item
              name="checkPassword"
              :rules="[{ required: true, validator: validateCheckPassword }]"
            >
              <a-input-password
            ...
            <a-form-item class="register-card__submit">
          ...
          <button class="ghost-btn" type="button" @click="router.push('/user/login')">
            已有账号？去登录
          </button>
        </div>
      </div>
...
.register-card :deep(.ant-input-affix-wrapper .ant-input::placeholder) {
...
.register-card :deep(.ant-form-item-with-help .ant-form-item-explain-error) {
  color: #ff7a9c;
}
